...
public: /** FWAppの実装 **/
	...
	virtual void Init(int argc, char* argv[]) {
		...
		***/
		...
		/// 床用の形状
		CDBoxDesc bd;
		bd.boxsize = ShapeScale() * Vec3d(40, 2, 60);
		shapeFloor = GetSdk()->GetPHSdk()->CreateShape(bd)->Cast();
		shapeFloor->SetStaticFriction(0.4f);
		shapeFloor->SetDynamicFriction(0.4f);
		...
		// 形状の作成
		bd.boxsize = ShapeScale() * Vec3f(3.0, 5.0, 3.0);
		shapeBox = GetSdk()->GetPHSdk()->CreateShape(bd)->Cast();
		shapeBox->SetDensity(1.0f);
		...
		for (int i = 0; i < numScenes; i++) {
			...
			GetPHScene()->SetGravity(Vec3f(0.0f, -9.8f * 2.0, 0.0f));	// 重力を設定
			GetPHScene()->SetTimeStep(0.05);
			GetPHScene()->SetNumIteration(60);//default 15
